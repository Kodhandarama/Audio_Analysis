...
def swipe(x, fs, plim, dt, sTHR):
    ...
    # Determine P2-WSs
    logWs = np.round(np.log2(np.multiply(4 * K, (np.divide(float(fs), plim)))))
    ws = np.power(2, np.arange(logWs[0], logWs[1], -1))   # P2-WSs
    pO = 4 * K * np.divide(fs, ws)   # Optimal pitches for P2-WSs
    # Determine window sizes used by each pitch candidate
    d = 1 + log2pc - np.log2(np.multiply(4 * K, (np.divide(fs, ws[0]))))
    ...
    for i in range(0, len(ws)):
        ...
        [X, f, ti,img] = plt.specgram(xk, NFFT=int(ws[i]), Fs=fs, window=w, noverlap=int(o))
        ...
        if i == (len(ws) - 1):
            ...
            k = np.arange(0, len(j))  # transpose added by KG
            #k = np.transpose(k1)
